 ...
 */
export async function retrieveCart(id?: string): Promise<CartWithInventory | null> {
    try {
        ...
        const productMap = new Map<string, { product: Product | null, fetched: boolean }>();
        ...
        await Promise.all(
            Array.from(productMap.keys()).map(async (productId) => {
                try {
                    ...
                    productMap.set(productId, { product: response.product, fetched: true });
                } catch (err) {
        ...
        const enrichedItems = await Promise.all(
            cart.items.map(async (item) => {
                ...
                return {
                    ...
                    is_in_stock: !variant.manage_inventory || variant.inventory_quantity > 0
                };
            })
        );
...
export async function getOrSetCart(countryCode: string): Promise<CartWithInventory | null> {
    ...
    if (cart?.region_id !== region.id) {
        const updatedCart = await sdk.store.cart.update(cart.id, { region_id: region.id }, {})
